...
//			mediaSourceId:	$('#mediaSource').val().toLowerCase(),
//			F2:				$('#paid').val()
//				 lpPage:			$('#lpPage').val(),
//				 innerMedia: 		$('#innerMedia').val(),
		...
		$.ajax({
			type : 'POST',
			url : 'http://paservice.adks.cn/api/common/add_puhui_basic_applyinfo',
//			url : '/do/rsploan/firstStep',
			data : params_first,
			dataType : 'json',
			async : false,
			success : function(response) {
				if (response.id > 0) {
					top.location.href = 'step1.html?flowId='+response.id;
